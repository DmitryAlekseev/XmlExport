...
type
  ...
  private
    ...
    { Возвращает сформированное имя файла для текущего набора данных,
      вида: Directory\FilenameFieldValue(инкремент).xml }
    function GetFilename(ADirectory, AFieldName: string; AInc: integer): string;
    { Возвращает сформированное имя каталога для текущего набора данных,
      вида: Directory\GroupingFieldName_GroupingFieldValue }
    function GetGroupingDir(ATemplateDir, AFieldName: string): string;

    { Парсинг входящего xml под набор данных }
    procedure XmlParse(AQuery: TFDQuery; ANode: TXmlNode);

    { процедуры для Synchronize-вызовов }
    procedure Active;
